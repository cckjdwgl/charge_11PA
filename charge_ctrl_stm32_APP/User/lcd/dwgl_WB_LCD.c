...
//   GPIO_Init(GPIOF, &GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;    //控制LCD_RS
  GPIO_Init(GPIOD, &GPIO_InitStructure);
...
// 	
  w.FSMC_AddressSetupTime = 0x01;							                  //地址建立时间：0x1
  w.FSMC_AddressHoldTime = 0x01;							                  //地址保持时间：0x00
  w.FSMC_DataSetupTime = 0x03;							                    //数据建立时间：0x2
  w.FSMC_BusTurnAroundDuration = 0x00;
  w.FSMC_CLKDivision = 0x00;
  w.FSMC_DataLatency = 0x03;
  w.FSMC_AccessMode = FSMC_AccessMode_B;

	r.FSMC_AddressSetupTime = 0x01;							                   //地址建立时间：0x1
  r.FSMC_AddressHoldTime = 0x01;							                   //地址保持时间：0x00
  r.FSMC_DataSetupTime = 0x07;							                     //数据建立时间：0x2
  r.FSMC_BusTurnAroundDuration = 0x00;
  r.FSMC_CLKDivision = 0x00;
  ...
  FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAM1;	   	//扩展NOR BANK 的第1个子BANK
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM1, ENABLE); 							//使能FSMC_Bank1_NORSRAM4 内存
...
// 	DH=(reg>>8);
// 	DL=(reg&0XFF);
//  	GPIO_ResetBits(SPI2_CS_PORT, SPI2_CS_PIN);
// 	GPIOSPI_Send_BYTE(DH);
// 	GPIOSPI_Send_BYTE(DL);
//  	GPIO_SetBits(SPI2_CS_PORT, SPI2_CS_PIN);
// 	
// // 	GPIO_SetBits(LCD_RS_PORT, LCD_RS_PIN);  //写数据 
//  	GPIO_ResetBits(LCD_RS_PORT, LCD_RS_PIN);	//写命令
...
// 	DL=(data&0XFF);
// // 	GPIO_ResetBits(SPI2_CS_PORT, SPI2_CS_PIN);
// 	  GPIOC->BRR = SPI2_CS_PIN;

// 	GPIOSPI_Send1_BYTE(DH);
// 	GPIOSPI_Send1_BYTE(DL);
// // 	GPIO_SetBits(SPI2_CS_PORT, SPI2_CS_PIN);
// 	  GPIOC->BSRR = SPI2_CS_PIN;

// // 	GPIO_SetBits(LCD_RS_PORT, LCD_RS_PIN);  //写数据 
// 	  GPIOC->BSRR = LCD_RS_PIN;
// // 	GPIO_ResetBits(LCD_RS_PORT, LCD_RS_PIN);	//写命令
// // 	GPIO_ResetBits(LCD_CS1_PORT, LCD_CS1_PIN);
// 	  GPIOB->BRR = LCD_CS1_PIN;
// // 	GPIO_ResetBits(LCD_WR_PORT, LCD_WR_PIN);
// 	  GPIOD->BRR = LCD_WR_PIN;
// // 	GPIO_SetBits(LCD_WR_PORT, LCD_WR_PIN);
// 	  GPIOD->BSRR = LCD_WR_PIN;
// // 	GPIO_SetBits(LCD_CS1_PORT, LCD_CS1_PIN);
// 	  GPIOB->BSRR = LCD_CS1_PIN;
// }
// void LCD_WR_REG(uint16_t reg)
// {
// // 	GPIO_SetBits(SPI2_MISO_PORT, SPI2_MISO_PIN);
...
//	LCD_WE_RAM(0x0054);
	...
	Delay_10ms(5);
	LCD_WR_REG(0x00E6);//设置像素时钟,33Mhz
//	LCD_WE_RAM(0x0004);
	Delay_10ms(1);
...
//	LCD_WE_RAM(0);
  ...
  LCD_WE_RAM((VDP>>8)&0x00FF);                    //Set VDP
	...
	if(SET_XY1==1)
	{
LCD_WE_RAM(0x0040);//dwgl				//横屏1
//LCD_WE_RAM(0x0004);//dwgl				//横屏1
//LCD_WE_RAM(0x0006);//dwgl				//横屏1
//LCD_WE_RAM(0x0002);//dwgl				//横屏1
//LCD_WE_RAM(0x0042);//dwgl				//横屏1
	}
	else
	if(SET_XY1==2)
	{
//	LCD_WE_RAM(0x0086);								//横屏2
	LCD_WE_RAM(0x0080);								//横屏2
	}
	else
	if(SET_XY1==3)
	{
	 LCD_WE_RAM(0x0023);//AC       //竖屏1
//	 LCD_WE_RAM(0x0020);//AC       //竖屏1
//	 LCD_WE_RAM(0x0024);//AC       //竖屏1
//	 LCD_WE_RAM(0x0030);//AC       //竖屏1
	}
	else
	if(SET_XY1==4)
	{
//	 LCD_WE_RAM(0x00AC);//AC       //竖屏2
//	 LCD_WE_RAM(0x00a0);//AC       //竖屏2
	 LCD_WE_RAM(0x0030);//AC       //竖屏2
	}
//LCD_WE_RAM(0x0000);//dwgl				//横屏1
//	LCD_WE_RAM(0x0086);						//横屏2
//	 LCD_WE_RAM(0x00AB);//AC       //竖屏1
//	 LCD_WE_RAM(0x00AC);//AC       //竖屏2
...
void LCD_InitAll(void)
{
	...
	GPIO_SetBits(LCD_CS2_PORT, LCD_CS2_PIN);
 	...
 	GPIO_ResetBits(LCD_CS2_PORT, LCD_CS2_PIN);	
//  	GPIO_SetBits(LCD_RS_PORT, LCD_RS_PIN);  //写数据 
//  	GPIO_SetBits(LCD_RD_PORT, LCD_RD_PIN);  //写数据 
//  	GPIO_SetBits(LCD_WR_PORT, LCD_WR_PIN);  //写数据 
	LCD_Init();
	GPIO_SetBits(LCD_CS1_PORT, LCD_CS1_PIN);
	GPIO_SetBits(LCD_CS2_PORT, LCD_CS2_PIN);	
}
