...
//			tft_1bitdeep_TXT (18, 0, LCD2_TxtBuffer,POINT_COLOR, BACK_COLOR,2);
    ...
    if(time_sys-time_sys_temp3 >= 2000)			//
		{
			State_Message(0,0,LGRAY,LGRAY,LGRAY);
			time_sys_temp3 =time_sys;
//			LCD_TEST();
		}
			...
			if(UART1_RXBUFFER[UART1_RXBUFFE_HEAD] == frame_headerC) 	//帧起始标志   
			{
				...
				if((UART1_Receive_Length >= 2)&&((UART1_Receive_Pointer[(UART1_RXBUFFE_HEAD+1)&UART1_RX_MAX]<<1) <= UART1_Receive_Length)) 	//长度刚好标志   
				{
					...
					if(last_d == frame_last)//校验对上
					{
						...
						State_Message(0, 0, NULL, GREEN,NULL);
...
//					BUS_Error_Byte(0x11);
		...
		else
		{
				...
				{
 					UART1_RX_State = 0x00;
// 					UART1_RXBUFFE_LAST &= UART1_RX_MAX;//最大字节
// 					UART1_RXBUFFE_HEAD = UART1_RXBUFFE_LAST;
					UART1_RXBUFFE_LAST = 0;//清空
					UART1_RXBUFFE_HEAD = UART1_RXBUFFE_LAST;
				}
		}
...
// 				UART1_RXBUFFE_HEAD &= UART1_RX_MAX;//最大字节
			...
			{
// 			global_u16temp = (UART1_RXBUFFER[(UART1_RXBUFFE_HEAD+1)&UART1_RX_MAX]);
// 			global_u16temp <<= 1;
// 			UART1_RXBUFFE_HEAD = global_u16temp;
// 		  UART1_RXBUFFE_HEAD &= UART1_RX_MAX;//最大字节
...
//          case 0xE2:  
			...
			global_u16temp = (UART3_RXBUFFER[(UART3_RXBUFFE_HEAD+1)&UART3_RX_MAX]);
			global_u16temp <<= 1;
			UART3_RXBUFFE_HEAD = global_u16temp;
//			UART3_RXBUFFE_HEAD += (UART3_RXBUFFER[(UART3_RXBUFFE_HEAD+1)&UART3_RX_MAX]<<1);
		...
		{
				UART3_RX_State = 0;
				if( (time_sys -time_uart3)>300 )
				{
					UART3_RXBUFFE_LAST = 0;//清空
					UART3_RXBUFFE_HEAD = UART3_RXBUFFE_LAST;
				}
				else
				{
