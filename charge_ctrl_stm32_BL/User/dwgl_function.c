...
void cmd_Device_num(void)
{
		...
		{
			en = 0xff;
			for(i=0;i<(UART1_RXBUFFER[(UART1_RXBUFFE_HEAD+6+i)&UART1_RX_MAX]+2);i++)  //ÅÐ¶ÏÒª²»ÒªÖØÐ´
			{
		...
		tft_DisplayStr(15, 125, device_num,POINT_COLOR, BACK_COLOR,1);
		}
		else
		if(LCDC.LCD1SPPID==0)
		{
		...
		tft_DisplayStr(270, 125, device_num,POINT_COLOR, BACK_COLOR,1);
		}
		...
		tft_DisplayStr(15, 125, device_num,POINT_COLOR, BACK_COLOR,2);
		}
		else
		if(LCDC.LCD2SPPID==0)
		{
		...
		tft_DisplayStr(270, 125, device_num,POINT_COLOR, BACK_COLOR,2);
		}
}
//---------------------------------------------------------------------------------
void cmd_ShakeHands(void)
{
	...
	u8 temp;
		...
		if(UART1_RXBUFFER[(UART1_RXBUFFE_HEAD+5)&UART1_RX_MAX] ==0x10)
		{
				if(device.Version[0]==Version_FLAG1)
				{
				temp = UART1_RXBUFFER[(UART1_RXBUFFE_HEAD+5)&UART1_RX_MAX];
...
//		if(UART1_RXBUFFER[(UART1_RXBUFFE_HEAD+5)&UART1_RX_MAX] == 130)
		...
		if((device.Version[0]==Version_FLAG1)&&((PRO_State&4)==4))   //ÔÚBÀàÖÐÓÐAÀà
		{
		UART1_TXBUFFER[7] ='E';//ÎÞÐ§°æ±¾
		}
